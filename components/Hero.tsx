    ...
    <section className="relative h-screen flex flex-col items-center justify-center text-center px-4 overflow-hidden z-10">
      <style>{`
        @keyframes tinsel-crack {
          ...
          30% {
            clip-path: polygon(0% 0%, 40% 10%, 60% 0%, 100% 10%, 100% 40%, 70% 50%, 100% 60%, 100% 100%, 60% 90%, 40% 100%, 0% 90%, 30% 50%, 0% 40%);
          }
          60% {
            clip-path: polygon(10% 10%, 30% 5%, 50% 15%, 80% 5%, 90% 30%, 60% 40%, 85% 60%, 70% 80%, 40% 70%, 20% 90%, 5% 60%, 25% 45%, 5% 25%);
            opacity: 0.4;
          }
          100% {
            clip-path: polygon(20% 20%, 25% 22%, 22% 25%, 30% 30%, 28% 32%, 35% 35%, 30% 40%);
            opacity: 0;
        ...
        .tinsel-overlay {
          position: absolute;
          inset: 0;
          background: linear-gradient(45deg, rgba(0,255,234,0.1), rgba(127,29,29,0.1));
          pointer-events: none;
          z-index: -1;
        }
        .tinsel-line {
          ...
          width: 120%;
          height: 2px;
          background: linear-gradient(90deg, transparent, #00ffea, #7f1d1d, transparent);
          box-shadow: 0 0 15px #00ffea;
          left: -10%;
          animation: tinsel-crack 8s infinite ease-in-out;
        }
      ...
      <div className="space-y-6 max-w-4xl relative">
        ...
        <p className="text-lg md:text-xl text-slate-400 font-light max-w-2xl mx-auto leading-relaxed">
          Мандарины по 3 €/кг, ёлка за 80 €, фейерверки под запретом. 
          Пока миллионы украшают дома, мы считаем последствия. 
        ...
        <div className="glass-morphism p-6 rounded-2xl inline-block border border-cyan-500/20">
          <p className="text-xs uppercase tracking-widest text-cyan-500 mb-2">До конца потребительского НГ осталось</p>
          <div className="flex gap-4 md:gap-8 justify-center items-center">
            {[
              ...
              <div key={i} className="flex flex-col">
                ...
                <span className="text-[10px] uppercase text-slate-500">{unit.label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="pt-12">
          ...
          >
            <span className="text-xs uppercase tracking-widest font-bold">Узнать правду</span>
            <ChevronDown className="animate-bounce w-8 h-8" />
          </button>
        </div>
      </div>

      {/* Tinsel crack effect overlay */}
      <div className="absolute inset-0 pointer-events-none opacity-20 bg-[url('https://www.transparenttextures.com/patterns/broken-noise.png')]"></div>
    </section>
  );
};
